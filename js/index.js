...
var pars = 
[
    "A build process that spits out an unreproducible binary is worrying. If what comes out of a build depends on the lunar cycle, the world becomes a hard place to reason about. This means that gratuitous use of other potentially changeable information should be kept to an absolute minimum in the source files.",

    "The build must work perfectly all the time it must be reliable. If it falls over every other day or occasionally produces a broken binary, then it is worse than useless—it’s dangerous. How can you be sure that you’re testing a good binary? How can you be sure that your company is releasing an acceptable product? Problems with the build system really hamper development.",

    "This idea is based on the theories of peripheral learning. If students are surrounded by new vocabulary and take it in through all of their senses, they will have a better chance of learning and remembering words or phrases. The pocket reminder uses the sense of touch as a learning tool.",
    
    "Our focus continues to be on teaching you the techniques and approaches you need to be successful in programming interviews. We reinforce these by illustrating the thought process that leads to the solution of each of the problems we present, and show you how to move forward when you’re stuck.",
    
    "Ahmad was assigned as the new road engineer for the city. His first task was to install police stations along the road connecting the center of the city and the governor house. The governor suggested to Ahmad N points along the road were the government can build police stations. Ahmad needs to choose at most K points to build police stations in them.",
    ...
    "These skills overlap with general coding skills, but they’re not the same; we’ve seen great coders crash and burn in programming interviews because they haven’t developed their interview skills. Early in our careers we crashed and burned a couple times ourselves, but you can avoid that by beginning your preparation with this book.",

    "Shahhoud is so experienced that he can find out the difficulty of any problem as soon as he reads it. He found out that the easiest problem in the contest has a difficulty of D. Shahhoud’s problem solving skill is S, and he knows that he can solve the problem if and only if his skill is higher or equal to the problem’s difficulty.",
    ...
    "Saeed is planning to watch the UEFA Champions League game tonight. Sadly, not all the pixels in the TV screen are working, and some of them are just black non-working dots! Saeed decided not to give up, and focus his eyes on a rectangular area of the total TV screen. This area might have bad pixels, that’s why Saeed decided that he would choose a rectangle that has no more than K black non-working pixels.",
    ...
    "Testing an application can never identify all of the bugs potentially present. Because of this, there will always be errors in deployed apps. As a developer, employing defensive strategies such as unit and regression testing can help to identify bugs early, but there are many factors that developers cannot always plan for.",
    
    "For example, a new device may ship with a unique screen configuration for which the application developer may not have accounted. It is important that when bugs do crop up in applications, developers can reproduce and isolate them in a systematic way, and release an update to users in a timely fashion.",
    
    "The rich amount of input available to the programmer (such as location and social media account access) enables applications on mobile platforms to be highly context aware. For example, an application might suggest restaurants nearby the user, tailored to their preferences retrieved from a social media account."
